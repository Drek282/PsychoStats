...
// +----------------------------------------------------------------------+
//
// $Id: class_Color.php 366 2008-03-17 17:05:38Z lifo $
...
class Image_Color
{
    ...
    *   @author     Jason Lotito <jason@lehighweb.com>
    */
    function getRange ( $degrees=2, $lightness=0 )
    {
        if ( $degrees == 0 )
            $degrees = 1;
        ...
        */
        ...
        if ($lightness){
        	...
        	$ch1 = rgb2hsv($this->color1);
        	$ch2 = rgb2hsv($this->color2);
        	
//        	echo "<pre>" . htmlentities(var_export($ch1, TRUE)) . "</pre>";
//        	echo "<pre>Hue [red]:		" . htmlentities(var_export($ch1[0], TRUE)) . "</pre>";
//        	echo "<pre>Sat [red]:		" . htmlentities(var_export($ch1[1], TRUE)) . "</pre>";
//        	
//        	echo "<pre>" . htmlentities(var_export($ch2, TRUE)) . "</pre>";
//        	echo "<pre>Hue [green]:		" . htmlentities(var_export($ch2[0], TRUE)) . "</pre>";
//        	echo "<pre>Sat [green]:		" . htmlentities(var_export($ch2[1], TRUE)) . "</pre>";
        	...
        	// echo "<pre>" . htmlentities(var_export($hdiff, TRUE)) . "</pre>";
        	/*
        	$cl = array(
        		round(abs($this->color2[0] - $this->color1[0]) / 2),
        		round(abs($this->color2[1] - $this->color1[1]) / 2),
        		round(abs($this->color2[2] - $this->color1[2]) / 2)
        	);
        	$ttt = rgb2hsv($cl);
        	$eee = $this->hsv2rgb($ttt[0], $ttt[1], $ttt[2]);
        	echo "<pre><b>etalon:</b>\n" . htmlentities(var_export($cl, TRUE)) . "</pre>";
        	echo "<pre><b>ttt:</b>\n" . htmlentities(var_export($ttt, TRUE)) . "</pre>";
        	echo "<pre><b>eee:</b>\n" . htmlentities(var_export($eee, TRUE)) . "</pre>";
        	$lightness = 255 - array_sum($cl) / 3;
        	$ccc = $this->rgb2hex($cl);
        	$boo = $this->rgb2hex($eee);
        	echo "<div style='width: 100px; height: 50px; background: #{$boo}; border: 1px dashed; margin: 10px'>&nbsp;#$boo&nbsp;</div>";
        	echo "<div style='width: 100px; height: 50px; background: #{$ccc}; border: 1px dashed; margin: 10px'>&nbsp;#$ccc&nbsp;</div>";
        	*/
        }
        ...
        for ( $x = 0; $x < $degrees; $x++ )
        {
            ...
            if ( $lightness )
            {
            	...
            	$pct = $delta/$degrees * $mod;
	           	// echo "Delta: $delta<br>PCT: $pct%<br>Mod: $mod<hr>";
	           	array_walk( $newcolor, '_changeLightness', $pct*100 );
              // $newcolor = rgb2hsv(array($newcolor[0], $newcolor[1], $newcolor[2]));
            }
            ...
            $newcolor = $this->hsv2rgb($ch1[0] + $hshift, $ch1[1], $ch1[2] + $bshift);
    ...
    */
    function hsv2hex ( $h, $s, $v )
    {
        ...
        if ( $s == 0 )
        {
            ...
            /* 1 */	$p = (integer)($v * (1.0 - $s));
            /* 2 */	$q = (integer)($v * (1.0 - $s * $f));
            /* 3 */	$t = (integer)($v * (1.0 - $s * (1.0 - $f)));
...
// }}}
		...
		function _changeLightness ( &$xx, $yy, $degree=0 )
    {
      if ( ( $xx + $degree ) < 256 )
      {
          if ( ( $xx + $degree ) > -1 )
   ...
   function rgb2hue($R, $G, $B)
   {
         ...
         if (($R >= $G) && ($G >= $B)) $H = 30*($G-$N)/($L-$N);
         if (($G >= $R) && ($R >= $B)) $H = 30+30*($L-$R)/($L-$N);
         if (($G >= $B) && ($B >= $R)) $H = 60+30*($B-$N)/($L-$N);
         if (($B >= $G) && ($G >= $R)) $H = 90+30*($L-$G)/($L-$N);
         if (($B >= $R) && ($R >= $G)) $H = 120+30*($R-$N)/($L-$N);
         
         if (($R >= $B) && ($B >= $G)) $H = 300+60*($L-$B)/($L-$N)-1;
	      }
	     return $H;
		}
